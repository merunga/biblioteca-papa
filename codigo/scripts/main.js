(function($) {
  ...
  var LunrSearch = (function() {
    ...
    LunrSearch.prototype.initialize = function() {
      ...
      this.loadIndexData(function(data) {
        self.entries = $.map(data.documentStore.store, function(raw, url) {
          return self.createEntry(raw, url);
        });
    ...
    LunrSearch.prototype.loadIndexData = function(callback) {
      ...
      $.getJSON(this.dataUrl, function(data) {
        self._data = data;
        $.getJSON(self.indexDataUrl, callback);
      });
      
    };

    LunrSearch.prototype.createEntry = function(raw, url) {
      var entry = $.extend({
        id: url
      }, raw);

      $.extend(entry, this._data[url]);
    ...
    LunrSearch.prototype.displayResults = function(entries) {
      ...
      if (entries.length === 0) {
        $entries.append('<p>'+this.notFoundMessage+'</p>');
      } else {
  ...
  $.fn.lunrSearch.defaults = {
    ...
    dataUrl   : '/js/data.json',   // Url for the .json file containing search index data
